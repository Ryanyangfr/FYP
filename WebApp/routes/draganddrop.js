...
router.get('/getAllDragAndDrop', (req,res) => {
    const dragNDropQuery = 'SELECT MISSION_TITLE, DRAGANDDROP_QUESTION, DRAGANDDROP_QUESTION_OPTION, DRAGANDDROP_QUESTION_ANSWER FROM DRAG_AND_DROP, DRAG_AND_DROP_OPTION, MISSION WHERE DRAG_AND_DROP.DRAGANDDROP_ID = DRAG_AND_DROP_OPTION.DRAGANDDROP_ID AND DRAG_AND_DROP.MISSION_ID = MISSION.MISSION_ID';
    const response = [];
        ...
        if (data.length != 0) {
            data.forEach((row) => {
                if (row.DRAGANDDROP_QUESTION !== currentQuestion) {
                    response.push({question: currentQuestion, title: currentTitle, options: options});
                    options = [];
