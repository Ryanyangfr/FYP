...
router.get('/getAllSubmissionURL', (req, res) => {
  ...
  const query = 'SELECT SUBMISSION_IMAGE_URL, QUESTION, HOTSPOT_NAME FROM SUBMISSION AS S,SUBMISSION_QUESTION AS SQ, TRAIL_HOTSPOT AS TH WHERE TEAM_ID = ? AND TRAIL_INSTANCE_ID = ? AND S.SUBMISSION_QUESTION_ID = SQ.QUESTION_ID AND TH.MISSION_ID = SQ.MISSION_ID';
