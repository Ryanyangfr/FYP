...
router.get('/getAllSubmissionURL', (req, res) => {
  ...
  const query = 'SELECT SUBMISSION_IMAGE_URL, QUESTION, HOTSPOT_NAME FROM SUBMISSION AS S, SUBMISSION_QUESTION AS SQ, MISSION AS M WHERE TEAM_ID = ? AND TRAIL_INSTANCE_ID = ? AND S.SUBMISSION_QUESTION_ID = SQ.QUESTION_ID AND M.MISSION_ID = SQ.MISSION_ID';
  ...
  conn.query(query, [team, instance_id], (err, rows) => {
    if (err) {
      ...
      const drawing_query = 'SELECT SUBMISSION_IMAGE_URL, QUESTION, HOTSPOT_NAME FROM SUBMISSION AS S, DRAWING_QUESTION AS DQ, MISSION AS M WHERE TEAM_ID = ? AND TRAIL_INSTANCE_ID = ? AND S.DRAWING_QUESTION_ID = DQ.QUESTION_ID AND M.MISSION_ID = DQ.MISSION_ID';
