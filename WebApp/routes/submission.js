...
router.get('/getAllSubmissionURL', (req, res) => {
  ...
  const query = 'SELECT SUBMISSION_ID, TEAM_ID, SUBMISSION_IMAGE_URL, QUESTION, HOTSPOT_NAME, S.SUBMISSION_STATUS FROM SUBMISSION AS S, SUBMISSION_QUESTION AS SQ, TRAIL_HOTSPOT AS TH WHERE TEAM_ID = 1 AND TRAIL_ID = (SELECT TRAIL_ID FROM TRAIL_INSTANCE WHERE TRAIL_INSTANCE_ID = ?) AND S.SUBMISSION_QUESTION_ID = SQ.QUESTION_ID AND TH.MISSION_ID = SQ.MISSION_ID;'
