...
router.get('/getWordSearchWords', (req, res) => {
  ...
  const query = 'SELECT M.MISSION_ID, HOTSPOT_NAME FROM TRAIL_HOTSPOT AS TH, WORDSEARCH AS WS WHERE TRAIL_ID = (SELECT TRAIL_ID FROM TRAIL_INSTANCE WHERE TRAIL_INSTANCE_ID = ?) AND TH.MISSION_ID = WS.MISSION_ID';
