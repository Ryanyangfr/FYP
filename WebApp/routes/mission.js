...
router.get('/getMissionQuizHistory', function(req,res){
    ...
    // var hotspot = req.query.hotspot;
    var query = 'SELECT * FROM MISSION_HISTORY,SUMMARY_TABLE WHERE MISSION_HISTORY.MISSION_ID IN (SELECT MISSION_ID FROM QUIZ_HISTORY) AND MISSION_HISTORY.ID = SUMMARY_TABLE.MISSION_ID AND TRAIL_INSTANCE_ID = ?';
