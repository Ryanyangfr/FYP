...
export default {
    ...
    methods:{
        ...
        getQuizOptions(quiz_question, quiz_id, quiz_answer){
            console.log('quiz id: ' + quiz_id)
             axios.get('//13.229.115.32:3000/quiz/getQuizOptionsHistory?quizID=' + quiz_id)
            .then(response =>{
    ...
    mounted(){
        ...
        axios.get('//13.229.115.32:3000/quiz/getQuizQuestionHistory?mission=' + this.$store.state.selectedQuizID)
        .then(response =>{
            var data = response.data;
            console.log('data: ')
            console.log(data)
